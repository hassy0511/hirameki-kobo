...
assert(audio.DEFAULT_BGM_VOLUME > 0 && audio.DEFAULT_BGM_VOLUME < 1, 'BGM初期音量が不正です');
assert(audio.MAX_BGM_GAIN >= 0.08 && audio.MAX_BGM_GAIN <= 0.12, 'BGM最大GainがiPadで聞き取れる実用範囲ではありません');
assert(audio.MAX_BGM_GAIN * audio.DEFAULT_BGM_VOLUME * 0.34 >= 0.009, 'BGM初期音量が実質無音になる水準です');
...
manager.configure({ master: true, bgm: true, bgmVolume: 0.35, mode: 'workshop', visible: true });
...
assert(duckTargets.some(value => value > 0 && value < audio.MAX_BGM_GAIN * 0.35), '効果音時にBGMがダッキングされません');
...
manager.configure({ master: true, bgm: true, bgmVolume: 0.35, mode: 'workshop' });
