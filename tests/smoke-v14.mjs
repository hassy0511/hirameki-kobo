...
assert(firstVisit.appElement.innerHTML.includes('1ねんせい さんすう'), '初回画面にG1コースがありません');
...
assert(firstVisit.appElement.innerHTML.includes('ルミナが ねむっている'), '初回ストーリー導入がありません');
...
const adminState = courses.createDefaultState();
adminState.introSeen = true;
adminState.courseChosen = true;
adminState.activeCourseId = 'g1';
adminState.settings.storyRevision = story.STORY_VERSION;
adminState.courses.g1.introSeen = true;
adminState.courses.g1.storySeen[story.storyKey('course', 'g1', 'main')] = true;
const adminHarness = createAppHarness({ savedRaw: adminState, seedStart: 830000 });
const progressBeforeAdmin = JSON.stringify(adminHarness.app.getState().progress);
assert.equal(adminHarness.app.isUnlocked(10, 'number'), false, '通常モードで未到達ステージが開いています');
adminHarness.dispatchAction('toggle-admin-unlock');
assert.equal(adminHarness.app.getState().settings.adminUnlockG1, true, '管理者モードをオンにできません');
for (const lineId of courses.COURSES.g1.lineOrder) {
  for (let stageIndex = 0; stageIndex < 11; stageIndex += 1) {
    assert(adminHarness.app.isUnlocked(stageIndex, lineId), '管理者モードで ' + lineId + '/' + (stageIndex + 1) + ' が開きません');
  }
}
assert.equal(JSON.stringify(adminHarness.app.getState().progress), progressBeforeAdmin, '管理者モードが学習進捗を書き換えています');
assert(adminHarness.appElement.innerHTML.includes('かんりモード'), '管理者モード中であることが画面に表示されません');
adminHarness.dispatchAction('toggle-admin-unlock');
assert.equal(adminHarness.app.getState().settings.adminUnlockG1, false, '管理者モードをオフにできません');
assert.equal(adminHarness.app.isUnlocked(10, 'number'), false, '管理者モードをオフにしても解放状態が残っています');
assert.equal(JSON.stringify(adminHarness.app.getState().progress), progressBeforeAdmin, '管理者モード解除時に学習進捗が変わりました');
